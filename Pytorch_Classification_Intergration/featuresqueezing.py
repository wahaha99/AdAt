...
class FeatureSqueezing:
    ...
    def learn_threshold_m(self, net, size, ori_learn, attacked_learn, fpr):
        ...
        center_attacked = np.mean(e_attacked_arr)
        #th = (center_ori + center_attacked) / 2
        selected_distance_idx = int(np.ceil(len(ori_learn) * (1 - fpr)))
        th = sorted(e_ori_arr)[selected_distance_idx-1]
        print("threshold: ", th)
        self.threshold_m = th

    def median_smooth_detection(self, size = 3, fpr=0.1):
        # intialize
        ad_count_ori = 0
        ad_count_attacked = 0
        # load model
        ...
        ori_detection = self.ori_arr[n_ori_learn:]
        attacked_detection = self.attacked_arr[n_attacked_learn:]
        self.learn_threshold_m(net, size, ori_learn, attacked_learn, fpr)
        # detection
        print('### testing threshold ###')
        for i in range(len(ori_detection)):
            image = ori_detection[i]        
    ...
    def joint_detection(self, bit_depth = 5, size = 3, fpr=0.1):
        # intialize
        ad_count_ori = 0
        ad_count_attacked = 0
        # load model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.7, help='lr')
    parser.add_argument('--fpr', type=float, default=0.1, help='fpr')
    parser.add_argument('--ifgsm', type=str2bool, default=False, help='ifgsm')
    ...
    if args.ifgsm == False:
        ...
        if args.median_smoothing == True:
            ...
            fq.median_smooth_detection(args.size, args.fpr)
        if args.joint == True:
        ...
        attacked_arr = []
        for i in range(266):
            ori_img = '/content/gdrive/MyDrive/EQ2445/Pytorch_Classification_Intergration/my_data/Master project data/attacked_image_case2_ifgsm/' + str(i) + '_img.png'
            attacked_img = '/content/gdrive/MyDrive/EQ2445/Pytorch_Classification_Intergration/my_data/Master project data/attacked_image_case2_ifgsm/' + str(i) + '_att.png'
            ori_img_tensor =in_transform(Image.open(ori_img))
            attacked_img_tensor =in_transform(Image.open(attacked_img))
            ori_arr.append(ori_img_tensor)
            attacked_arr.append(attacked_img_tensor)
        fq = FeatureSqueezing(img_path, mean, std, model_path, num_classes, ori_arr, attacked_arr, args.lr, 1)
        if args.color_depth == True:
            print('########## bit depth ##########')
            fq.color_bit_detection(args.bit)
